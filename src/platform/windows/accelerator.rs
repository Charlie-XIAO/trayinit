...
pub fn to_accel(accelerator: &Accelerator, command: u16) -> Result<ACCEL, AcceleratorError> {
    // Reference:
    ...
    let vk_code = key_to_vk(accelerator.key())?;
...
pub fn fmt_label(accelerator: &Accelerator, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    // Reference: muda/src/platform_impl/windows/accelerator.rs::Display for
    // Accelerator.
    let modifiers = accelerator.modifiers();
    if modifiers.contains(Modifiers::CONTROL) {
        write!(f, "Ctrl+")?;
...
fn key_to_vk(key: Code) -> Result<VIRTUAL_KEY, AcceleratorError> {
    // Reference: muda/src/platform_impl/windows/accelerator.rs::key_to_vk.
    Ok(match key {
        Code::KeyA => VK_A,
        Code::KeyB => VK_B,
        Code::KeyC => VK_C,
